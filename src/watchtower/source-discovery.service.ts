 ...
 *
 * Templates:
 *  meusanimes:  https://meusanimes.blog/e/<slug>-1-episodio-<n>/
...
export class SourceDiscovery {
  ...
  async candidates(
    ...
    for (const sourceId of order) {
      const urls = this.urlsFor(sourceId, animeSlug, episodeNumber);
  ...
  allCandidates(animeSlug: string, episodeNumber: number): SourceCandidate[] {
    const out: SourceCandidate[] = [];
  ...
  private urlsFor(sourceId: string, slug: string, ep: number): string[] {
    const base = sourceEpisodeUrl(sourceId, slug, ep);
    if (!base) return [];
